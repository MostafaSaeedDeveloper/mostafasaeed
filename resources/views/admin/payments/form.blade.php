...
<div class="card"><div class="card-body"><div class="row g-3">
    <div class="col-md-6"><label class="form-label">Invoice</label><select name="invoice_id" class="form-select" required>@foreach($invoices as $invoice)<option value="{{ $invoice->id }}" @selected(old('invoice_id', $payment->invoice_id)==$invoice->id)>{{ $invoice->formatted_number }} - {{ $invoice->client?->name }}</option>@endforeach</select></div>
