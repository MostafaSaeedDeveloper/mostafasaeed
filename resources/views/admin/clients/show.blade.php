...
<div class="row g-3">
    ...
    <div class="col-md-6"><div class="card"><div class="card-header">Invoices</div><ul class="list-group list-group-flush">@forelse($client->invoices as $invoice)<li class="list-group-item d-flex justify-content-between"><span>{{ $invoice->formatted_number }}</span><span>{{ number_format($invoice->total, 2) }}</span></li>@empty<li class="list-group-item text-muted">No invoices.</li>@endforelse</ul></div></div>
</div>
@endsection
